...
> Tiếp theo `10-project-vua-drf-vua-ninja-extra.md`. Từ bài này, các chủ đề không xuất hiện trong `article_writer.py` (file đã dùng xuyên suốt bài 01-10), nhưng có dùng thật ở feature app khác trong `leadplusone_api` — vẫn cần biết vì sẽ gặp khi đọc/viết endpoint mới.
