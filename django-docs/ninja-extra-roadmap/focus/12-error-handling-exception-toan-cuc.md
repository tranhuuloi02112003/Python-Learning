...
Bài tiếp theo (`13-route-context-va-settings.md`) đi vào `self.context` (đối tượng `RouteContext` mọi controller đều có) và cách Django `settings.py` cấu hình ninja/ninja-extra ở cấp toàn cục.
