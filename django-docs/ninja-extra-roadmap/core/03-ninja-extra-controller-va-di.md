...
Mỗi khi có request tới, ninja-extra không tự gọi `TaskController()` như 1 class Python bình thường — nó tạo controller qua 1 dependency injector chạy ngầm (chi tiết cơ chế thật, verify bằng source code, để ở bài `14-dependency-injection-thuc-su-qua-injector.md`). Điều cần biết ngay ở bài này: injector đó vẫn luôn gọi `__init__` của bạn, nên có 2 cách viết đều hợp lệ.
