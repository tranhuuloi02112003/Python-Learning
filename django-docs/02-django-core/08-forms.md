# 08. Rào chắn An ninh (Forms & Validations)
...
## 5. Ghi chú Forms từ project Todo hiện tại

Phần này gom các điểm dễ nhầm đã gặp trong `TaskForm`, `ProjectForm`, template modal và views.

### A. `request.POST` là gì?

Khi user submit form, Django nhận dữ liệu trong:

```python
request.POST
```

`request.POST` không phải dict Python thường. Nó là `QueryDict`, dạng giống dict nhưng hỗ trợ một field có nhiều value.

Ví dụ browser submit:
...
Django nhận gần giống:

```python
request.POST["name"]    # "Website"
request.POST["status"]  # "active"
```

### B. `class Meta` trong `ModelForm`
...
`class Meta` trong `ModelForm` nói cho Django biết:

```txt
Form này dựa trên model nào.
User được submit những field nào.
Field đó render ra widget gì nếu có khai báo widgets.
```
...
`widgets` chỉ quyết định cách field render ra HTML.
...
Nó không quyết định database.

### D. `ChoiceField` và `TextChoices`

Trong `projects/choices.py`:
...
`TextChoices` là enum của Django cho field có choices.

Mỗi dòng gồm:

```txt
giá trị lưu trong DB, nhãn hiển thị cho user
```
...
- DB lưu: `"active"`
- Form/template hiển thị: `"Active"`

Trong form:

```python
status = forms.ChoiceField(choices=ProjectStatus.choices)
```

Django render dropdown từ choices đó.

### E. `is_valid()` và `cleaned_data`
...
`form.is_valid()` làm các việc:

```txt
Kiểm tra required.
Kiểm tra kiểu dữ liệu.
Kiểm tra choices hợp lệ.
Chạy các custom validation như clean_title().
```

Nếu form valid, Django tạo:

```python
form.cleaned_data
```

Đây là dict dữ liệu đã validate. Chỉ nên dùng `cleaned_data` sau khi `form.is_valid()` trả `True`.

### F. `form.save()`

Với `ModelForm`, có thể dùng:

```python
form.save()
```

Nếu form không có `instance`, Django tạo object mới:
...
Nếu form có `instance`, Django update object đang có:
...
Trong project hiện tại, một số nơi dùng service:

```python
services.create_project(form.cleaned_data)
services.update_project(project, form.cleaned_data)
```

Mục tiêu là để logic thay đổi dữ liệu đi qua `services.py`.

### G. Browser validation và Django validation

Browser validation:

```txt
Browser tự chặn form nếu input required bị rỗng.
Hiện popup mặc định như "Please fill out this field".
```
...
Django chạy form.is_valid().
Lỗi nằm trong form.errors hoặc form.<field>.errors.
```

Nếu muốn browser không chặn và để Django xử lý lỗi, thêm:

```html
<form novalidate>
```

Trong project hiện tại, modal cần UX riêng nếu muốn form lỗi tự mở lại modal sau khi server render.

### H. `messages.success()` và `messages.error()`
...
Đây là Django messages framework.

Ý nghĩa:

```txt
Lưu thông báo tạm vào request/session.
Sau redirect, template đọc biến messages để hiển thị.
```

### I. `redirect()` và hidden `next`

```python
return redirect("projects:detail", pk=project.pk)
```

`"projects:detail"` là URL name. Nếu `project.pk = 5`, Django tạo URL:
...
```txt
Gửi kèm URL hiện tại khi submit form.
Sau khi xử lý xong, view redirect user về đúng trang cũ.
```
...
Nếu có `next`, quay lại URL đó. Nếu không có, fallback về `tasks:all`.

### J. Template form fields và include
...
- `form.title`: render input HTML cho field title.
- `form.title.errors`: render lỗi của field title nếu form invalid.

Tạo URL từ URL name:

```django
{% url "projects:edit" project.id as project_edit_url %}
```

Include template con và truyền biến:

```django
{% include "projects/components/project_form_modal.html" with form_action=project_edit_url %}
```
