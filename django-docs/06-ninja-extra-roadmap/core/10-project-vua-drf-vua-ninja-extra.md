...
Bài 01-10 đã đủ để đọc hiểu `article_writer.py` từ đầu tới cuối. Các bài tiếp theo (`11` trở đi) đi vào những chủ đề của Ninja/ninja-extra mà file này không dùng tới, nhưng vẫn cần biết vì có thể gặp ở feature app khác trong repo, hoặc cần dùng khi viết endpoint mới. Bắt đầu từ `focus/11-query-va-form-data.md`.
