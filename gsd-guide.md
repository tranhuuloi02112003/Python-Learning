...
|               | `/gsd-pr-branch`         | Lọc sạch commit liên quan đến `.planning`, tạo nhánh code sạch để push GITHUB.|
...
**Giai đoạn 6: Hoàn tất & Đẩy code lên Remote Repo**

- **Chuyển Phase**: Dùng `/gsd-next` để tiếp tục làm các Phase sau (ví dụ: Phase 2 kết nối API thật).
- **Tạo nhánh sạch để Review (Cực kỳ quan trọng)**: Khi tính năng đã xong và bạn muốn đẩy code lên repo GitHub/GitLab của công ty. Bạn **KHÔNG** nên `git push` nhánh hiện tại ngay, vì nó đang lưu rất nhiều commit nháp của hệ thống lập kế hoạch GSD. Hãy gõ lệnh:
  👉 `/gsd-pr-branch`
  _Tác dụng_: Lệnh này sẽ tự động nhân bản nhánh làm việc hiện tại, đi lùi về quá khứ để **lọc và xóa sạch 100% các commit có liên quan đến AI (thư mục `.planning/`)**. Nó sẽ trả lại cho bạn một nhánh code hoàn toàn "tinh khiết" (chỉ gồm frontend code của Level Assessment) để bạn tự tin `git push` lên.
...
_Tình huống: Đang làm bài ngon ơ thì sếp bảo "Ê em, làm thêm cho anh cái giao diện này trước đi". Cấu trúc roadmap bị phá vỡ!_
...
Hệ thống sẽ tự động quét "não" nó: _"À, làm tiếp việc hôm qua đang dở -> Nó sẽ tự điều hướng sang lệnh `/gsd-resume-work` hoặc `/gsd-quick` tương ứng"_ mà bạn không cần phải lục lọi trí nhớ.
