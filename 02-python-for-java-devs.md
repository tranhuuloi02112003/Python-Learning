...
> *(Mở rộng: Trong Java bạn phải khai báo `String name = "Gemini"`, còn Python biến sẽ tự nhận kiểu từ giá trị gắn cho nó. Tuy nhiên, Python sẽ báo lỗi nếu bạn cố tình cộng chuỗi với số mà không ép kiểu rõ ràng).*
...
> *(Lưu ý: Không dùng `camelCase` như `userName` hay `calculateTotal` trừ những trường hợp đặc biệt không thể tránh).*
...
> *(Thay vì dùng một cặp `{}` rỗng như các ngôn ngữ khác).*
...
> *(Liên tưởng: Giống `ArrayList` hoạt động co giãn linh động)*
...
# Lọc List hiện tại và biến đổi, chỉ với 1 dòng ngọn gàng
numbers = [1, 2, 3, 4, 5]
...
> *(Liên tưởng: Một Array "chỉ đọc" hoặc dùng để lưu các hằng số toạ độ)*
...
> *(Liên tưởng: Tương tự như `HashMap` hoặc `ObjectJSON`)*
...
> *(Liên tưởng: Bạn hay dùng nó như `HashSet`. Set dùng `{}` giống dict nhưng không có cấu trúc theo cặp Key:Value).*
...
> *(Tránh viết hàm đè - Overloading rườm rà)*
...
> *(Python thực chất ngầm gói gọn vào đối tượng Tuple lại cho bạn)*
```python
def get_user_status():
...
> *(Liên tưởng: `self` giống hệt `this` trong Java/C++, nhưng nó bắt-buộc-phải-hiện-diện rõ ràng trên danh sách tham số argument của mọi class methods nhé).*
- **Access Modifier (Private/Public):** Python thực sự **chỉ có Public**. Mọi biến và hàm nào bắt đầu với gạch dưới đơn `_name` thì mọi lập trình viên tự quy ước nó là *internal/private*, không nên dùng trực tiếp từ bên ngoài.
...
Dù là kiểu động, các dự án Backend lớn tại Python bắt buộc dùng *Type Hinting* (có từ Python 3.5). Nó không bắt buộc trình biên dịch, nhưng các IDE và công cụ linter sẽ dùng nó để cảnh báo lỗi y như bạn viết code kiểu tĩnh.
> *(Java developer sẽ rất thích tính năng này vì nó ngăn ngừa bug ngay khi code).*
...
> *(Liên tưởng: giống hệt cụm try / catch / finally).*
...
Cách tốt nhất của Python để quản lý tài nguyên cần *đóng/mở* (như Đọc/Ghi file, Database Connection). Blocks `with` cam kết rằng tài nguyên sẽ tự động đóng lại khi kết thúc khối lệnh, dù cho trong quá trình chạy có bị báo lỗi văng Exception đi chăng nữa.
> *(Liên tưởng: Giống hệt `try-with-resources` trong ngôn ngữ kiểu tĩnh).*
...
> *(Dành cho Java Dev: Sự khác biệt này trái ngược 100% với Java: ở Java `==` làm nhiệm vụ so object reference, còn `.equals()` làm việc so value. Giờ trong Python nó đảo lại: `==` giống như tính năng `.equals()` cũ).*
